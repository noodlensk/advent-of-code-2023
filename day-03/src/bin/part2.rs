...
fn part1(input: &str) -> String {
    ...
    let mut gears_idx: HashMap<(usize, usize), (usize, Vec<usize>)> = HashMap::new();
    ...
    for (&i, v) in &numbers_matrix {
        for (&j, &v2) in v {
            for (x, y) in coordinates_to_around(i, j, format!("{v2}").len(), max_x, max_y) {
                ...
                if val == '*' {
                    let e = gears_idx.entry((x, y)).or_default();

                    e.0 += 1;

                    e.1.push(v2 as usize);
                }
            }
        }
    }

    let sum: usize = gears_idx
        .iter()
        .filter(|(_, (first, _))| *first >= 2)
        .map(|(_, (_, vec))| vec.iter().product::<usize>())
        .sum();
